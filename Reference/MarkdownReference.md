...
**Markdown** is created by [Daring Fireball](http://daringfireball.net/); the original guideline is [here](http://daringfireball.net/projects/markdown/syntax). Its syntax, however, varies between different parsers or editors. **SEUITE** is using [GitHub Flavored Markdown][GFM].
...
A paragraph is simply one or more consecutive lines of text. In markdown source code, paragraphs are separated by two or more blank lines. In SEUITE, you only need one blank line (press `Return` once) to create a new paragraph.
...
In SEUITE, input ‘#’s followed by title content, and press `Return` key will create a header.
...
In SEUITE, inputting ‘>’ followed by your quote contents will generate a quote block. SEUITE will insert a proper ‘>’ or line break for you. Nested block quotes (a block quote inside another block quote) by adding additional levels of ‘>’.
...
SEUITE only supports fences in GitHub Flavored Markdown. Original code blocks in markdown are not supported.
...
You can find more details [here](http://support.SEUITE.io/Math/).
...
The full syntax for tables is described below, but it is not necessary to know the full syntax in detail as the markdown source code for tables is generated automatically by SEUITE.
...
SEUITE now supports [YAML Front Matter](http://jekyllrb.com/docs/frontmatter/). Input `---` at the top of the article and then press `Return` to introduce a metadata block. Alternatively, you can insert a metadata block from the top menu of SEUITE.
...
In SEUITE, they will be rendered like so:
...
In SEUITE, clicking the link will expand it for editing, and command+click will open the hyperlink in your web browser.

### URLs

SEUITE allows you to insert URLs as links, wrapped by `<`brackets`>`.

`<i@SEUITE.io>` becomes <i@SEUITE.io>.

SEUITE will also automatically link standard URLs. e.g: www.google.com.
...
If you’re using markdown for building websites, you may specify a URL prefix for the image preview on your local computer with property `SEUITE-root-url` in YAML Front Matters. For example, input `SEUITE-root-url:/User/Abner/Website/SEUITE.io/` in YAML Front Matters, and then `![alt](/blog/img/test.png)` will be treated as `![alt](file:///User/Abner/Website/SEUITE.io/blog/img/test.png)` in SEUITE.

You can find more details [here](https://support.SEUITE.io/Images/).
...
SEUITE recommends using the `*` symbol.
...
Some websites provide iframe-based embed code which you can also paste into SEUITE. For example:
